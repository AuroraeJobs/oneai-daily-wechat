...
当前 OneAI Daily 推送统一使用以下来源展示模式：

- 正文每条新闻只显示来源信息，例如：`**来源：** Reuters, “Article title”, 2026-07-07.`
- 正文不显示裸 URL。
- 正文不放每条新闻的 `阅读原文` 按钮。
- 正文不显示 `原文链接：URL`。
- 草稿字段 `content_source_url` 保持为空，不绑定公众号底部官方 `阅读原文`。

发布脚本会在生成微信公众号 HTML 时自动清理来源段落中的 URL，只保留来源说明。Markdown 源文件里可以保留原始 URL 供内部溯源，但最终公众号正文不会显示该 URL。
